...
echo '
This is a quick summary of what we have seen so far. Press space to read further
...
And Execute them on the Terminal by issuing:

./filename.sh

The name of your shell script must end with a .sh . 
This lets the user know that the file is a shell script. This is not compulsary but is the norm.


3. Variables
------------

To Create a variable and assign its value:
...
Variables are non-datatyped (a variable can hold strings, numbers, etc. with out being defined as such).
...
read makes your shell script interactive. read will let the user enter values while the script is being executed. When a program encounters the read statement, the program pauses at that point. Input entered through the keyboard is read into the variables following read, and the program execution continues.


4. Quote Marks
--------------

Regular double quotes make the shell ignore whitespace and count it all as one argument being passed or string to use. Special characters inside are still noticed/obeyed.

Single quotes make the interpreting shell ignore all special characters in whatever string is being passed.
